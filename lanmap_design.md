...
| G | **状態 (死活)** | 自動 | 🟢 up / ⚪ Down (`Last Seen` 併記) |
| H | **応答速度 (RTT)** | 自動 | 直近Pingの往復時間（簡易ネットワーク品質可視化）。<br>🟢 `<15ms`（高速・有線/良電波） / 🟡 `15〜60ms`（通常） / 🟠 `>60ms`（混雑・遅延） |
| I | **固定IP** | 手動 | 管理用チェックボックス（DB保存のみ） |
| J | **監視状態** | 自動表示 | 🟢 監視中 / ⏸️ 一時停止 / ⚪ 未監視 のバッジ |
| K | **操作** | UIボタン | 各行末の `...` アクションメニュー（ドロップダウン） |
