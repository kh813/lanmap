...
1. **軽量ポートスキャンエンジン (`internal/scanner/ports.go`)**:
   * 主要な代表的サービスポート（21:FTP, 22:SSH, 53:DNS, 80:HTTP, 443:HTTPS, 445:SMB, 554:RTSP, 631:IPPプリンタ, 3389:RDP, 5000:Synology/NAS, 8008:Google Cast, 8080:HTTP-Alt 等）を対象に、タイムアウト 60ms 程度の超低負荷・非侵入型 TCP Connect プローブを実施。
   * 検出結果を `open_ports` カラムに保存（例: `"22:SSH,80:HTTP,443:HTTPS"`）。
2. **UIホバー表示 (Tailwind / CSS Group Hover)**:
   * マウスオーバー時に瞬時にカードを表示。クリック不要で端末の詳細属性・オープンポート・通信状況を俯瞰可能。
