...
### 11.2 完了済み機能 (Completed in v0.0.18)
...
* **1端末複数アドレスの統合管理**:
  ...
  * `hosts` テーブルに `ipv6_addresses TEXT`（JSONまたはカンマ区切り）を追加し、代表IP（主要グローバルIPv6またはIPv4）をメインテーブルに表示しつつ、ホバーカードや詳細モーダルで全IPv6アドレス（種別・有効期限付き）を展開表示する。

### 14.3 パッシブ検出エンジン（マルチキャスト監視）
IPv6にはブロードキャストが存在しないため、マルチキャスト通信をパッシブに傍受する。

1. **ICMPv6 NDP (Neighbor Discovery Protocol) パッシブ傍受**:
   - IPv4のARPに相当するプロトコル。
   - 端末接続時の重複アドレス検出（DAD: Duplicate Address Detection）や近隣解決で送出される **Neighbor Solicitation (NS) / Neighbor Advertisement (NA)** パケットをパッシブ監視し、0秒で「MACアドレスとIPv6アドレスの対応」を瞬時に特定。
2. **DHCPv6 パッシブ傍受 (UDP 546/547)**:
   - クライアントがDHCPv6サーバーを探すマルチキャスト（`ff02::1:2`）を傍受し、動的割り当て端末（🔵）として自動認識。
3. **Rogue RA（不正ルーター広告）検知（セキュリティ強化）**:
