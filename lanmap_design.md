...
|---|---|---|---|
| A | **IPアドレス** | 自動 | ICMP Ping / 手動登録から取得。<br>・固定IP帯の未承認時: `⚠️` アイコン＆行を赤色ハイライト（侵入警戒）<br>・DHCP動的帯の端末: `ℹ️` アイコン＆通常行背景（日常の動的接続） |
...
| G | **承認ステータス**| 手動 / 自動 | ・🟢 **承認済**（管理下端末）<br>・🔵 **DHCP 🆕**（DHCP動的帯の新規端末・水色）<br>・ℹ️ **DHCP動的**（DHCP動的帯の既存端末・ブルーグレー）<br>・🆕 **NEW**（固定IP帯の未知端末・強い警告パルス）<br>・🔴 **未承認**（固定IP帯の未承認端末・赤色警戒） |
