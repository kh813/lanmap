...
| K | **監視状態** | 自動表示 | 🟢 監視中 / ⏸️ 一時停止 / ⚪ 未監視 のバッジ |
...
ブラウザで画面を開いたまま放置していても、バックグラウンドで実行された定期スキャン結果やUptime Kuma同期状況が常にリアルタイムに反映されるよう、HTMX の定期ポーリング機構を実装する。
...
1. **7日間 Ping 時系列履歴 & SVG スパークラインチャート (`ping_history`)**:
   ...
   * **SVG スパークライン & Uptime Kuma 風ステータスバー**: 外部 JS ライブラリ不要で、ピュアなインライン SVG により直近7日間の遅延変動・パケロス・稼働率を美しく可視化。
...
7. **Webhook URL / Uptime Kuma認証情報の暗号化保存**
   * `settings` テーブルに平文で保持しているWebhook URLやKuma接続情報を暗号化。
8. **簡易監査ログ**
