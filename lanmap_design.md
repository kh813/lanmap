...
4. **OS・機器プロファイル別 適応型（Adaptive）ポート選定エンジン**:
   * 全端末に一律 17 ポートを叩くのではなく、ステップ 1 で得られた属性（ICMP Ping TTL値、MAC OUIベンダー、mDNSモデル/ホスト名等）から端末プロファイルを自動判定し、関係のある最小限のポート（2〜4ポート）のみに限定してスキャンする。
   * **Mac / Apple端末**: Bonjour/mDNSをネイティブ動作しているため無駄なポートは除外し、リモートログイン（`22 SSH`）、ファイル共有（`445 SMB`, `548 AFP`）、およびWeb管理（`80/443`）を対象とする。攻撃や侵入の足がかりとなりやすいSSHの開放状況を把握し、インシデント発生時のトリアージ（SSH経由の侵入かどうかの特定）を可能にする。AirPlay等のメディアポートやWindows固有の `3389 RDP`、`9100 プリンタ` は叩かない。
   * **Windows端末**: `TTL=128` や NetBIOS から判定し、`445 SMB`, `3389 RDP`, `80/443 Web` のみを対象とする。Mac固有の `548 AFP` や `5000/7000 AirPlay` は叩かない。
   * **プリンタ / 複合機**: Canon, Epson, HP, Ricoh 等の OUI から判定し、`631 IPP`, `9100 RAW`, `80/443 Web管理` のみを対象とする。SSH や RDP などの侵入誤検知を招くポートは叩かない。
   * **ルーター / ネットワーク機器**: Buffalo, Cisco, Netgear 等の OUI から判定し、`80/443 Web管理`, `22 SSH`, `53 DNS` を対象とする。
   * **NAS / Linuxサーバー**: Synology, QNAP, Linux等から判定し、`80/443`, `22 SSH`, `445 SMB`, `5000/5001 DSM` を対象とする。
   * **モバイル / IoT**: iPhone, Android, スマート家電等は原則ポートスキャンを行わず、mDNS/SSDP のみで識別。
