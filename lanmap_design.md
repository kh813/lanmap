...
CREATE TABLE IF NOT EXISTS segments (
    ...
    dhcp_range VARCHAR(100) DEFAULT '',   -- DHCP動的割り当てIP範囲 (例: "100-200" または "192.168.1.100-192.168.1.200")
...
* **セグメントごとの DHCP IPレンジ設定 & Wi-Fi端末分布からの自動推定**: 各セグメントに動的IP帯域（例: `100-200` または `192.168.1.100-192.168.1.200`）を設定可能。RFC 2131 上の制約（DHCPパケット自体はプール全体の範囲を通知しない）を解決するため、`lanmap` が検出した Wi-Fi クライアント端末（スマホ・PC）のIPクラスタリングにより「💡 推定: 100〜200」を自動サジェストするほか、一般的なルーターのプリセットボタンを提供。
