...
|---|---|---|---|
| A | **IPアドレス** | 自動 | ICMP Ping / 手動登録から取得。<br>・固定IP端末: 稼働中 `🟢` / 停止中 `⚪`<br>・DHCP動的端末: 稼働中 `🔵` / 停止中 `⚪`<br>・未承認端末: 稼働中 `🟡`（アンバーハイライト） / 停止中 `⚪`<br>・同一IP旧端末履歴: `🕒 前回` バッジ表示 + `⚪`（半透明行）<br>・異常・警告（ストーム/VPN/リモート待受）: `⚠️`<br>・（将来予約）侵入検知・重大インシデント: `🔴` |
...
| G | **承認ステータス**| 手動 / 自動 | ・🟢 **承認済**（管理下端末）<br>・🔵 **DHCP 🆕**（DHCP動的帯の新規端末・自動許可）<br>・🔵 **DHCP動的**（DHCP動的帯の既存端末・自動許可）<br>・🆕 **NEW**（固定IP帯の未知端末・強い警告パルス）<br>・🟡 **未承認**（固定IP帯の未承認端末・アンバー警戒） |
...
### 11.2 完了済み機能 (Completed in v0.0.16)
...
1. **フェデレーション機能（分散拠点・マルチLAN統合監視 / `lanmap-agent` 連携）** (最優先 / 第13章参照)
