...
6. **i18n（多言語）対応**
   * v1.0時点ではWeb UIの表示文言は日本語固定とする。将来対応として日本語・英語の2言語をサポートし、`web/template/` 配下のテンプレート文言を翻訳キー化した上で `web/i18n/<lang>.json`（or `.yaml`）等の翻訳リソースファイルとして `go:embed` で内包する。言語切替は設定画面での明示選択、またはブラウザの `Accept-Language` ヘッダーによる自動判定を検討する。
7. **7日間 / 30日間の長期 Ping レスポンス・稼働率推移モーダル画面（個別ホスト詳細分析）**
   * 現在のホバーカード（デフォルト24時間表示）に加え、各ホストのアクションメニュー等から開く「7日間 / 30日間の長期時系列推移モーダル」を追加。時間帯別の詳細レイテンシ、パケットロス発生時間帯、稼働率推移を拡大グラフで詳細分析可能にする。
