...
外部ネットワークや監視対象端末に ESET, Windows Defender, 各種パーソナルファイアウォール・IDS/IPS が導入されている環境で、`lanmap` が攻撃ツールとして検知・遮断されることを防ぐため、**セーフモード（デフォルト）** および **オンデマンドポート診断** を提供する。

1. **セーフモード (Safe Mode / 低ノイズ監視 - デフォルト)**:
   * **背景**: 生存端末に対して17ポートのTCP SYN接続やHTTP/TLSプローブを定期的に一斉送信すると、ESET等のセキュリティソフトにより「Port Scan Attack」として検知・アラート発報される。
...
4. **OS・機器プロファイル別 適応型（Adaptive）ポート選定エンジン**:
   ...
   * **効果**: クローズドポートへの無駄な SYN パケットを根絶し、ESET などのパーソナルファイアウォール・IDS の検知閾値（接続拒否パケットの急増）に引っかからない安全なプロービングを実現する。
...
### 11.2 完了済み機能 (Completed in v0.0.13)
...
* **セーフモード（低ノイズ・IDS誤検知ゼロ監視 - デフォルト）& オンデマンドポート診断** (完了)
  * 定期スキャンは Ping + ARP + DNS/mDNS のみで静かに監視し、ESET等のセキュリティソフトによるポートスキャン誤検知を完全防止。
