...
7. **ポート分類とセキュリティ・サービスバッジ体系（UI・詳細モーダル）**:
   * 検出された開放ポートに基づき、セキュリティ警告（Risk）と役割情報（Info）を厳密に分離：
     - **🚨 VPNサーバー検出 (Critical / 即時対処)**: `1723` (PPTP), `5555` (SoftEther), `1194` (OpenVPN)
       - 社員や持ち込み端末が無断で外部から社内LANに穴を開ける野良VPN待受
   ...
   * ホスト詳細モーダルおよびホバーポップオーバーにて、サービス情報バッジ（Info）およびセキュリティリスクバッジ（Risk）をわかりやすく表示する。
