...
7. **📊 ホスト行クリック詳細モーダル & ⚡ オンデマンド即時 Ping 診断**
   * テーブルの行をクリックするだけで、**過去7日間の詳細な遅延推移グラフ（SVG）**、**時間帯別稼働ヒートマップ（4時間毎）**、**稼働率・平均/最小/最大RTT・ジッター統計**、およびその場で即時疎通チェックが打てる「⚡ 今すぐPing診断」を備えた詳細モーダルを表示。
8. **🔍 瞬時（0ms）リアルタイム・キーワード検索 & フィルタリング**
   * IP、ホスト名、表示名、メーカー、モデル、OS、ポート番号、接続形態（Wi-Fi/有線）、承認状態などの複合キーワードで即座にインクリメンタル絞り込み。
9. **🔌 有線LAN / 📶 Wi-Fi 接続形態の独立列表示**
