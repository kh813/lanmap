...
18. **🚨 リモートアクセス・VPN待受監視 & サービス情報バッジ（Info）体系**
    * **リモートアクセス & VPN監視の強化**: 業務LAN内で不正に立ち上げられたVPNサーバー（`SoftEther 5555`, `PPTP 1723`, `OpenVPN 1194`）やリモート操作・平文ログイン待ち受け（`Telnet 23`, `Windows RDP 3389`, `macOS画面共有/VNC 5900`, `TeamViewer 5938`, `AnyDesk 7070`）を適応型プロファイルでピンポイント監視し、`🚨 VPN` や `⚠️ リモート待受` のセキュリティ警告バッジを付与。
