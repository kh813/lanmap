...
18. **🚨 リモートアクセス・VPN待受監視 & デイリー低ノイズ適応型巡回（ランダムジッター方式）**
    * **リモートアクセス & VPN監視の強化**: 業務LAN内で不正に立ち上げられたVPNサーバー（`SoftEther 5555`, `PPTP 1723`, `OpenVPN 1194`）やリモート操作待ち受け（`Windows RDP 3389`, `macOS画面共有/VNC 5900`, `TeamViewer 5938`, `AnyDesk 7070`）を適応型プロファイルでピンポイント監視。
    ...
    * **セキュリティリスク警告バッジ**: 危険ポートが開いている端末には、一覧画面や詳細モーダルで `🚨 VPN`、`⚠️ リモート待受` などの警告バッジを表示。
