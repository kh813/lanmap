...
- [x] **14.3 Web UI テンプレート (`web/template`)**
  ...
  - [x] `main_table.html` 改修：
    - [x] 行背景: DHCP動的端末はハイライトから除外（日常利用端末のアラート疲れ防止）、未承認端末はアンバー淡色ハイライト
    - [x] IP横アイコン: 固定IP稼働中は `🟢`、停止中は `⚪`、DHCP端末は `🔵`、未承認端末は `🟡`、異常・警告は `⚠️`、将来の侵入検知用に `🔴` を予約
    - [x] 承認列バッジ: `🔵 DHCP 🆕`（新規・自動許可）、`🔵 DHCP動的`（既存・自動許可）、`🟡 未承認`
- [x] **14.4 ドキュメント更新**
  - [x] `README.md`、`lanmap_design.md`、`lanmap_todo.md` 更新
...
### 🔹 Phase 18: リモートアクセス・VPN待受ポート拡充 & デイリー低ノイズ適応型巡回（ランダムジッター方式）
- [x] **18.1 プロファイル別ポートマップ拡充 & 低レートプローブ実装 (`internal/scanner/ports.go`)**
  - [x] TeamViewer (`5938`), AnyDesk (`7070`), VNC/macOS画面共有 (`5900`), SoftEther (`5555`), PPTP (`1723`), OpenVPN (`1194`) を各プロファイルに追加
  - [x] 低レート・ディレイ付きプローブ関数（ポート間に100msウェイトを挟む `ScanOpenPortsLowNoise`）の実装
- [x] **18.2 DB層 (`internal/db`)**
  - [x] `hosts` テーブルに `last_port_scan` (最終ポートスキャン日時), `next_port_scan` (次回スキャン予定日時) を追加 & マイグレーション
  - [x] `GetDuePortScanHost()`（オンラインかつ現在時刻 >= `next_port_scan` のホストを1台ずつ順次取得）
  - [x] `UpdateHostPortScanSchedule()`（スキャン結果更新 & 次回予定日を 24時間 ± ランダムジッターで設定）
- [x] **18.3 スキャナー層 (`internal/scanner`)**
  - [x] 定期Pingスキャン時、セーフモード下でも期限到来（`Due`）のオンライン端末に対して 1台ずつ順次低ノイズ検査を実行するバックグラウンドタスク
  - [x] 初回発見端末に対する初回低ノイズ適応型検査のスケジュール
- [x] **18.4 UI & i18n (`internal/i18n`, `web/template`, `internal/web`)**
  - [x] リスクポート（VPN: 1723/5555/1194, リモート操作: 3389/5900/5938/7070, リモートログイン: 22/23）に対する警告バッジ・ラベル定義
  - [x] メインテーブルおよびホスト詳細モーダルでのリスク表示
  - [x] 日英辞書キー追加
- [x] **18.5 単体テスト & 総合検証**
...
### 🔹 Phase 19: DHCP パケット・パッシブ監視 & ゼロ秒即時検知エンジン
- [x] **19.1 DHCP (BOOTP) パケット解析モジュール (`internal/monitor/dhcp.go`)**
  - [x] BOOTP ヘッダーパーサー（Magic Cookie `0x63, 0x82, 0x53, 0x63`、`chaddr` からのMACアドレス抽出、`yiaddr` / `ciaddr`）
  - [x] DHCP Option パーサー（Option 53 メッセージ種別、Option 50 要求IP、Option 12 ホスト名、Option 60 Vendor Class、Option 55 Parameter List）
  - [x] Option 60 / 55 による OS 種別（Windows, Android, Linux, Apple等）の推定ロジック
  - [x] OUI データベース照合によるメーカー名解決 (`scanner.LookupVendor`)
- [x] **19.2 パッシブ UDP リスナー (`internal/monitor/dhcp.go`)**
  - [x] UDP 67 (DHCP Server / ブロードキャスト宛先) & UDP 68 (DHCP Client) の非特権データグラムリッスン
  - [x] ポート制限・競合時の Graceful フォールバック（警告ログ出力、他機能への影響ゼロ）
  - [x] バックグラウンド常駐タスク起動 (`cmd/lanmap/main.go`)
- [x] **19.3 DB 自動登録・`is_dhcp` 連動 & 即時アラート (`internal/db`, `internal/monitor`)**
  - [x] `FindSegmentForIP` によるセグメント自動割当
  - [x] `is_dhcp = 1`, `status = 'up'` での `UpsertHostOnScan` 連動（既存端末の `is_dhcp` 確実更新）
  - [x] ホワイトリスト未登録の新規端末に対する Webhook 即時通知連携
  - [x] セグメントの DHCP レンジ自動学習連動 (`AutoAdjustSegmentDHCPRange`)
- [x] **19.4 単体テスト & 総合検証**
  - [x] DHCPDISCOVER / DHCPREQUEST / DHCPACK パケットパーサー単体テスト (`dhcp_test.go`)
  - [x] パケット受信からの DB 登録・更新・所属セグメント割当テスト
  - [x] `go test -v ./...` & `go build ./cmd/lanmap` 検証

---

### 🔹 Phase 20: OS別 監視ポートカスタマイズ & CSVインポート/エクスポート機能 (Phase 25にて完全実装・完了)
- [x] **20.1 データベース層 (`internal/db`)** (完了 -> Phase 25.1 参照)
  - [x] `custom_profile_ports` テーブル追加 & マイグレーション (`id`, `profile_id`, `protocol`, `port`, `protocol_name`, `description`, `is_enabled`, `is_builtin`, `created_at`, `updated_at`)
  - [x] 組み込みデフォルトポート一覧の初期シード投入ロジック
  - [x] ポート定義 CRUD メソッド (`ListProfilePorts`, `CreateProfilePort`, `UpdateProfilePort`, `DeleteProfilePort`, `ToggleProfilePort`, `ResetProfilePortsToDefault`)
  - [x] DB層単体テスト作成 & 実行
- [x] **20.2 スキャナー連動 & 動的ポートマップ解決 (`internal/scanner`)** (完了 -> Phase 25.2 参照)
  - [x] `internal/scanner/ports.go` のリファクタリング: 静的 `profilePortMaps` から DB 連携の動的マップ取得への移行
  - [x] ポート設定のインメモリキャッシュ機構 (`sync.RWMutex`)
  - [x] プロファイル別適応型ポートスキャンでの動的ポート定義適用テスト
- [x] **20.3 CSV インポート & エクスポート (`internal/ports` または `internal/web`)** (完了 -> Phase 25.3 参照)
  - [x] CSVエクスポート処理（ヘッダー: `TargetOS,Protocol,Port,ProtocolName,Description,Enabled`）
  - [x] CSVパーサー & バリデーション（OS種別、プロトコル、ポート番号範囲 1-65535、文字数制限）
  - [x] インポート時のマージ / 完全置換処理
- [x] **20.4 Web UI & タブコンポーネント (`web/template`, `internal/web`)** (完了 -> Phase 25.4 参照)
  - [x] `settings_modal.html` のタブ化（`[ ⚙️ システム設定 ]` / `[ 🎯 監視ポート設定 ]`）
  - [x] 監視ポート設定タブ UI 実装（OSプロファイル別フィルター、ポート一覧テーブル、編集/削除ボタン、有効/無効トグル）
  - [x] 新規ポート追加モーダル / フォーム
  - [x] `[ 📥 CSVエクスポート ]` & `[ 📤 CSVインポート ]` ボタンおよびファイルアップロードハンドラー
  - [x] `[ 🔄 デフォルトにリセット ]` 機能
  - [x] 日英（EN/JA）辞書キー追加 (`internal/i18n`)
- [x] **20.5 単体テスト & 総合検証** (完了 -> Phase 25.5 参照)
  - [x] CSVインポート/エクスポート、UI操作、スキャナー連動のテスト
  - [x] `make test` & コミット

---

### 🔹 Phase 21: フェデレーション機能 (分散拠点・マルチLAN統合監視 & `lanmap-agent`) (Phase 27にて完全実装・完了)
- [x] **21.1 アーキテクチャ & 通信プロトコル設計 (`internal/federation`)** (完了 -> Phase 27.1 参照)
  - [x] Agent-Server 間のデータペイロード定義 (`AgentReport`, `AgentHostInfo`, `AgentMetadata`)
  - [x] 暗号化通信仕様（TLS + Bearer Token + SHA-256署名）
  - [x] セマンティックバージョニング照合 & スキーマ互換性ネゴシエーション仕様
- [x] **21.2 データベース層 (`internal/db`)** (完了 -> Phase 27.1 参照)
  - [x] `federation_agents` テーブル追加 & マイグレーション (`id`, `name`, `token_hash`, `remote_ip`, `cidr`, `status`, `version`, `schema_version`, `version_mismatch`, `last_seen_at`)
  - [x] `hosts` テーブルに `agent_id` カラム追加 & インデックス作成
  - [x] エージェント管理 CRUD メソッド (`CreateAgentPairingPIN`, `VerifyAndRegisterAgent`, `ListAgents`, `UpdateAgentStatus`, `DeleteAgent`, `UpdateAgentHeartbeat`)
  - [x] リモートホスト一括 Upsert メソッド (`UpsertRemoteHosts(agentID, hosts)`)
- [x] **21.3 Server 側 Web API & ペアリング処理 (`internal/web`, `internal/federation`)** (完了 -> Phase 27.2 参照)
  - [x] `POST /api/federation/pair/start`: 受付モード開始 & ワンタイムPIN (6桁, 15分有効) 発行
  - [x] `POST /api/federation/pair/request`: Agentからの参加要求受付 & 保留リスト登録
  - [x] `POST /api/federation/pair/approve`: Web UI からの管理者承認 & 永続トークン発行
  - [x] `POST /api/federation/report`: Agentからの定期JSONレポート受信、バージョン検証、DB反映
  - [x] スキーマ非互換時の `426 Upgrade Required` 拒絶ハンドリング
- [x] **21.4 Agent 側バイナリ・サブコマンド (`cmd/lanmap-agent` または `lanmap agent`)** (完了 -> Phase 27.4 参照)
  - [x] 軽量エージェントバイナリの実装（Web GUIなし、低フットプリント）
  - [x] ペアリングCLIコマンド (`lanmap agent pair --server <URL> --pin <PIN> --name <Name>`)
  - [x] ローカル設定ファイル（`agent.json`）の暗号化/安全な永続化
  - [x] バックグラウンド定期LAN探索 (Ping, ARP, DNS, ポート検査) & HTTPS Push 送信
  - [x] 通信ヘッダーへのバージョンメタデータ付与 (`X-Lanmap-Agent-Version`, `X-Lanmap-Schema-Version`)
- [x] **21.5 Web UI 表示 & バージョン整合性警告 (`web/template`, `internal/web`)** (完了 -> Phase 27.3 参照)
  - [x] 左サイドバー (`sidebar.html`) に「🌐 拠点LAN (Remote Sites)」セクション追加
  - [x] 拠点クリックによるメインテーブル表示切替（リモートホスト一覧フィルタリング）
  - [x] サーバー・エージェント間のバージョン不一致警告バッジ（サイドバー: `⚠️ v0.0.11 (要更新)`）
  - [x] メイン画面上部のバージョン不一致警告バナー表示（アップデート案内）
  - [x] 設定モーダル内に「🌐 フェデレーション管理」画面（エージェント追加、PIN発行、承認/拒否、ステータス一覧）追加
  - [x] 日英（EN/JA）辞書キー追加 (`internal/i18n`)
- [x] **21.6 単体テスト & 統合検証** (完了 -> Phase 27.5 参照)
  - [x] ペアリングフロー、トークン認証、レポート受信、バージョン照合、UI表示のテスト
  - [x] `make test` & コミット
...
### 🔹 Phase 24: IPv6 デュアルスタック・パッシブ監視サポート (Beta) (完了)
- [x] **24.1 データベース層 (`internal/db`)**
  - [x] `hosts` テーブルに `ipv6_addresses TEXT DEFAULT ''` カラム追加 & 自動マイグレーション
  - [x] `Host` 構造体への `IPv6Addresses` フィールドおよび `IPv6Info` ヘルパー追加
  - [x] `settings` に `enable_ipv4`（既定 true）/ `enable_ipv6`（既定 false: Beta）アクセサ追加 & バリデーション
  - [x] `UpsertHostByMAC`: 同一MACアドレスの既存ホストへの複数IPv6（LLA/GUA/ULA）の重複なしマージ集約ロジック
...
- [x] **24.3 Web UI テンプレート & 国際化 (`web/template`, `internal/i18n`, `internal/web`)**
  ...
  - [x] `main_table.html`: IP列への `[ 🔗 IPv6 ]` バッジおよび純粋IPv6端末表示
  - [x] `host_detail_modal.html`: 保持IPv6アドレス一覧（スコープ種別・生成方式）展開テーブル追加
  - [x] `handler.go`: 設定保存APIでの IPv4/IPv6 トグル受け取り & バリデーション
  - [x] Web統合テスト (`internal/web/web_test.go`)
- [x] **24.4 総合テスト & ビルド検証**
  - [x] `make test` 全テスト通過確認
  - [x] `make build` 動作確認
...
### 🔹 Phase 28: 設定モーダルUI最適化（幅拡大 & 1カラム行分割レイアウト）(完了)
- [x] **28.1 モーダル幅拡大 (`web/template/partials/settings_modal.html`)**
  - [x] モーダル幅を `max-w-4xl` (896px) から `max-w-5xl` (1024px) に拡大し、監視ポート設定の8列テーブルの横スクロールを解消
- [x] **28.2 システム設定タブの1カラム行分割レイアウト刷新**
  - [x] 2カラム化による上下の不自然な引き伸ばし・無駄な余白を完全解消
  - [x] 基本設定: テーマ表示、データ保持期間、システム更新、スキャンモード（3択均等カード）を整然と行分割配置
  - [x] Webhook通知: 4サービス（Google Chat, Slack, Teams, Discord）をバランスの良いカードグリッドで集約
  - [x] カスタムTLS: 証明書パスと秘密鍵パスをすっきりと配置
- [x] **28.3 ビルド & 検証**
  - [x] `make test` 全テスト通過確認
  - [x] `make build` 成功確認
...
### 🔹 Phase 29: Web管理画面での拠点エージェント（フェデレーション）設定機能 (完了)
- [x] **29.1 サーバー側 API エンドポイント (`internal/web`)**
  - [x] `POST /api/federation/agent/pair`: 親機サーバーへ参加要求送信 & 承認待機開始
  - [x] `GET /api/federation/agent/poll`: 承認ステータスポーリング & トークン受領・ローカル設定保存
  - [x] `POST /api/federation/agent/unpair`: 親機連携解除（ローカル設定消去）
  - [x] `POST /api/federation/agent/report`: 手動即時レポート送信トリガー
- [x] **29.2 Web UI テンプレート & 国際化 (`web/template`, `internal/i18n`)**
  - [x] `settings_modal.html`: 第3タブ「`[ 🌐 拠点エージェント (Agent) ]`」追加 & タブ切替ロジック
  - [x] 未連携（Standalone）フォーム: 親機URL、ワンタイムPIN、拠点名入力 & ペアリング開始ボタン
  - [x] 承認待機中表示: スピナー、承認待ちメッセージ、自動HTMXポーリング
  - [x] 連携中（Agent）カード: 接続状態、親機URL、拠点名、エージェントID、即時レポート送信ボタン、連携解除ボタン
  - [x] 日英辞書キー追加 (`internal/i18n`)
- [x] **29.3 単体テスト & 総合検証**
  - [x] Webハンドラー単体テスト (`internal/web/web_test.go`)
  - [x] `go test ./...` PASS確認
  - [x] `make build` 動作確認
...
### 📌 Milestone: v0.1.3 (定時サマリー通知 ＆ システム生存確認・ハートビート機能)
- [ ] **Daily / Weekly 定時サマリー通知（Scheduled Summary Report via Webhook）**
  - [ ] 指定時刻（例: 深夜 00:00 / 朝 08:00）または指定曜日（例: 毎週月曜朝）に、ネットワーク全体の稼働状況サマリーを Webhook（Slack/Teams/メール等）へ自動送信
  - [ ] **サマリー集計項目**:
    - 📊 総接続台数 / オンライン稼働台数 / セグメント別内訳
    - 🟡 前日（または過去1週間）の未承認端末・新規検知ホスト一覧
    - ⚠️ セキュリティリスク検出端末（VPNサーバー待受、リモートアクセス待受等）
    - 📉 監視対象ホストの障害発生・稼働率低下レポート
    - 🟢 システム稼働状態（lanmap プロセス稼働時間、DBサイズ、スキャン実行回数）
  - [ ] **動作生存確認（Heartbeat / ヘルスチェック）**: 定期通知の受信により、監視サーバーおよび通知パイプラインが正常に稼働し続けていることを管理者が受動的に確認可能
  - [ ] 設定画面の「通知設定」タブに「日次サマリー通知（有効/無効・送信時刻）」「週次サマリー通知（有効/無効・送信曜日/時刻）」の設定UIを追加
